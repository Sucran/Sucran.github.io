...
（注：技术术语处理说明：  
1. "binary cross entropy" 采用通用译名"二元交叉熵"而非直译"二进制交叉熵"  
2. "Vision Transformers" 保留专业缩写ViT的同时给出全称译名"视觉变换器"  
3. "max loss" 根据上下文技术含义译为"最大损失"而非字面"最大丢失"  
4. 被动语态"is supervised"主动化为"进行监督"符合中文技术文献表述习惯）
...
（注：专业术语处理说明：
1. "MLPs"保留英文缩写并添加中文全称"多层感知机"进行注解
2. "LLMs"同样保留缩写并补充"大语言模型"全称
3. "bias"在首次出现时标注"偏置"并保留英文原词，符合技术文献双语标注惯例
4. 通过"改进方案"、"运算"等术语保持技术文档的严谨性）
...
（注：MLPs采用"多层感知机"译法以保持技术术语准确性；"hyperparameter"统一译为"超参数"符合中文机器学习领域惯例；通过"范畴/本质上"等措辞在保持专业性的同时增强行文流畅度）
...
（注：根据技术文本特性进行了以下优化：  
1. "trivial"译为"轻松实现"而非字面直译，突出技术便捷性  
2. "visualization and understanding"采用"可视化与可解释性"这一CV领域标准术语  
3. "rationale"译为"判定依据"精准对应医学AI的决策解释需求  
4. 保留英文术语**加粗**格式确保技术概念辨识度）
...
另一种方法是**"钉子床法"**，即在放大后的输出中选择单一位置保留原始值，其余位置设为零。通过连续的卷积层处理，这些零值将逐渐被有效数据填充。
...
为生成边界框，算法会智能地将编码器输出的标记作为**Transformer解码器**的输入。通过引入可训练的查询参数——例如输入五个、十个或二十个查询，即可检测图像中相应数量的目标物体。解码器利用自注意力层和编码器输出的交叉注意力层，为每个查询生成数值。这些数值经由**前馈网络（FFN）**处理后，输出类别标签与边界框坐标，或标记为未检测到目标。  
...
（注：专业术语处理说明：
1. 保留CNN/ViT等缩写形式但添加中文全称注释
2. "activation maps"译为行业通用术语"激活映射图"
3. "attention map"采用学界通用译法"注意力图"
4. "pixel space"译为专业表述"像素空间"
5. 通过"架构"补译隐含的模型概念
6. "straightforward"根据计算机视觉领域表达习惯译为"直观"）
...
（注：专业术语处理说明：
1. "attention weights"保留专业表述"注意力权重"
2. "CNNs"采用中文领域通用译法"卷积神经网络"并保留英文缩写
3. "Grad-CAM"作为专有算法名称保留不译
4. "video understanding"译为"视频理解"符合计算机视觉领域术语规范）
